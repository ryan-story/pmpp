...
def test_pooling_accuracy(batch_size=2, channels=16, height=32, width=32, kernel_size=2):
...
def benchmark_pooling(batch_size=8, channels=64, height=128, width=128, kernel_size=2):
    ...
    print(f"Max Pooling - Custom: {custom_max_time:.3f}ms, PyTorch: {torch_max_time:.3f}ms")
    print(f"Avg Pooling - Custom: {custom_avg_time:.3f}ms, PyTorch: {torch_avg_time:.3f}ms")
