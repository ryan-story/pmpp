...
from nn.layers.activations import ReLU, Flatten
...
def mnist_cnn_example():
    ...
    model = Sequential([
        ...
        Conv2D(in_channels=1, out_channels=16, kernel_size=3, padding=1, name="conv1"),
        ReLU(),
        MaxPooling2D(kernel_size=2, stride=2, name="pool1"),  # Output: 16x14x14
        
        # Second convolutional layer
        Conv2D(in_channels=16, out_channels=32, kernel_size=3, padding=1, name="conv2"),
        ReLU(),
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            if (i + 1) % print_every == 0:
                ...
                print(f'Epoch [{epoch+1}/{num_epochs}], Step [{i+1}/{num_batches}], Loss: {avg_loss:.4f}, Accuracy: {accuracy:.2f}%')
