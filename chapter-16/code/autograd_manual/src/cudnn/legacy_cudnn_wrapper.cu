...
extern "C" int conv2d_forward(float* input, float* weights, float* bias, float* output, int batch_size, int in_channels,
    ...
    CHECK_CUDNN(cudnnGetConvolutionForwardAlgorithm(cudnnHandle, in_desc, fil_desc, conv_desc, out_desc,
...
extern "C" int conv2d_backward(float* input, float* weights, float* d_output, float* d_input, float* d_weights,
    ...
    CHECK_CUDNN(cudnnGetConvolutionBackwardDataAlgorithm(cudnnHandle, fil_desc, out_desc, conv_desc, in_desc,
    ...
    CHECK_CUDNN(cudnnGetConvolutionBackwardFilterAlgorithm(cudnnHandle, in_desc, out_desc, conv_desc, fil_desc,
