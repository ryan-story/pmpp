...
extern "C" int conv2d_forward(
    ...
    cudnnSetFilter4dDescriptor(fil_desc,  CUDNN_DATA_FLOAT, CUDNN_TENSOR_NCHW,
    ...
    CHECK_CUDNN(cudnnGetConvolutionForwardAlgorithm(
        cudnnHandle, in_desc, fil_desc, conv_desc, out_desc,
...
extern "C" int conv2d_backward(
    ...
    CHECK_CUDNN(cudnnConvolutionBackwardData(
        ...
        conv_desc,
        bwd_data_algo,
        d_ws_data, ws_data_size,
        &beta,
        in_desc, d_in_grad));

    // Backward Filter
    cudnnConvolutionBwdFilterAlgo_t bwd_fil_algo;
    CHECK_CUDNN(cudnnGetConvolutionBackwardFilterAlgorithm(
        cudnnHandle, in_desc, out_desc, conv_desc, fil_desc,
        CUDNN_CONVOLUTION_BWD_FILTER_PREFER_FASTEST,0,&bwd_fil_algo));
    size_t ws_fil_size=0;
    CHECK_CUDNN(cudnnGetConvolutionBackwardFilterWorkspaceSize(
        cudnnHandle, in_desc, out_desc, conv_desc, fil_desc,
        bwd_fil_algo,&ws_fil_size));
    void* d_ws_fil=nullptr; cudaMalloc(&d_ws_fil, ws_fil_size);
    ...
    cudaFree(d_in_grad); cudaFree(d_fil_grad); cudaFree(d_bias_grad);
    cudaFree(d_ws_data); cudaFree(d_ws_fil);
    cudnnDestroyTensorDescriptor(in_desc);
    cudnnDestroyTensorDescriptor(out_desc);
    cudnnDestroyTensorDescriptor(bias_desc);
    cudnnDestroyFilterDescriptor(fil_desc);
...
extern "C" int maxpool2d_forward(
    ...
    float *d_in, *d_out; int* d_idx;
    cudaMalloc(&d_in,  in_b);  cudaMalloc(&d_out, out_b); cudaMalloc(&d_idx, idx_b);
...
extern "C" int maxpool2d_backward(
    ...
    float *d_in, *d_out;
    cudaMalloc(&d_in, in_b); cudaMalloc(&d_out, out_b);
